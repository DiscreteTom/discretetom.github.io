...
1. 把2型文法变换为CNF
2. 对非终结符进行编号
3. 如果存在生成式```Ai -> Aj ω```其中Ai的编号大于Aj的编号，则把之前Aj的生成式代入，直到左侧非终结符Ai的编号小于右侧第一个非终结符的编号
...
采用形如```[q, z, p]```的非终结符，意为**一个串的集合**，里面的串满足：**从q状态出发，栈顶为z时，处理完串之后PDA将弹出栈顶z并变换到p状态**
...
通过这个转移函数可以看出，如果一个串“从状态q出发，在栈顶为z时，处理完串之后可以把z弹出去并转移到状态p”，也就是```[q, z, p]```。那么这个串能够被拆分成两部分，第一部分是字符a，自动机在p状态且栈顶为z时接收这个字符a，并把栈顶从z变成A，第二步“从状态r出发，在栈顶为A时，处理完剩下的串之后可以把A弹出去并转移到状态p”，也就是```[r, A, p]```
