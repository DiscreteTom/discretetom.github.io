...
**闪速存储器是目前唯一具有大容量、非遗实行、低价、可在线改写、较高速度的存储器**
...
假设模块存取一个字的存取周期为T，总线传送周期（即给一个模块发送存取信号的耗时）为t，那么令m = T / t称为**交叉存取度**，显然存储器的交叉模块数量应该等于交叉存取度m，以此实现效率最大化
