...
顺序控制字段用来执行下一条微指令的地址。本例中的微指令后四位直接给出了下一条微指令的地址，另外两位为**测试方式位**。如果测试方式有效，则根据直接地址和测试方式与**地址转移逻辑**共同决定下一条微指令的地址。详见地址转移逻辑部分
...
也叫**组合逻辑控制器**，被当前RISC计算机和高性能计算机普遍采用
...
上一条指令与下一条指令存在**时间**上的重叠，每一个时钟周期就能输出一个结果
